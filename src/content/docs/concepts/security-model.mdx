...
Cada instancia tiene su propio key store. Los `id_token` y `access_token` se firman con **RS256** por default. Las keys públicas se exponen vía JWKS en `https://<slug>.auth.prysmid.com/oauth/v2/keys`.
...
4. **Apuntar tu DNS**. `acme.auth.prysmid.com` lo apuntás vos (CNAME al server tuyo) o cambiás el issuer en tu app. Tus tokens emitidos antes siguen válidos hasta expirar.
