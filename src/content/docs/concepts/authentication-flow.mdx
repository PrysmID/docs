...
| `urn:zitadel:iam:org:project:roles` | Roles del usuario en el project Zitadel asociado a tu app. |
...
No vas a recibir `id_token` (no hay usuario), solo `access_token`. Ver también [machine keys para agentes](/agents/machine-keys/) si lo que querés es que un agente IA opere el plano de control.
