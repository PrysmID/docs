...
**`access_token` vs `id_token`.** The `id_token` is for your *backend* (proves user identity). The `access_token` is for talking to APIs (Zitadel or others). Don't mix them.

**Multi-tenant: validate `resourceowner`.** If you serve many tenants in one instance, `iss/aud/exp/sig` isn't enough — also check that `urn:zitadel:iam:user:resourceowner:id` matches the tenant the user is trying to access. Otherwise Bob from Acme could use his Acme token to access Globex resources in your app.
