...
- **Add** a passkey — one click launches Zitadel's native WebAuthn ceremony (Touch ID, Windows Hello, security key, etc.).
- **Remove** a passkey.
- **Remove** the authenticator app (TOTP).

<Aside type="note">
Today the page supports **add and remove**, not rename. Passkey rename is on the backlog as a fast-follow.
</Aside>
...
<Aside type="caution" title="Returning to your product">
There is **no** `return_to` parameter yet: when the user finishes enrolling, they stay on the Prysm:ID security page and return to your product by navigating back. Automatic return-to-product after enrollment is a planned fast-follow. If you need it sooner, say so and it gets prioritized.
</Aside>
