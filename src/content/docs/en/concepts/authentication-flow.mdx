...
- **`iss`**: the issuer. Must be exactly `https://<slug>.auth.prysmid.com`. If you validate against another string, you'll accept forged tokens.
...
You won't get an `id_token` (no user), just `access_token`. If what you want is for an AI agent to operate the control plane, see [Connect Claude Code →](/en/agents/quickstart-claude/) — the MCP server (`@prysmid/mcp`) uses device flow and delegates the permissions of the human who signed in, not separate machine credentials.
