---
title: Workspaces
description: The basic unit of the model. One workspace = one of your accounts = one dedicated instance.
---
...
- An **auth domain** (`acme.auth.prysmid.com`).
...
| State | What it means |
|---|---|
| `provisioning` | We just created the workspace. We're standing up the dedicated instance, configuring DNS, getting everything ready. Takes 60-90s. |
...
| `failed_provisioning` | Something failed at create. Recoverable without losing the slug; write us. |
...
<Aside type="note">
Workspace members are **human accounts on `app.prysmid.com`**, not end users. End users live inside the workspace's instance and are distinct.
</Aside>
