...
| 401 | `invalid_token` | Token expired or doesn't sign with the expected JWKS. If it's from the MCP, delete `~/.config/prysmid-mcp/token.json` and let the server re-cache via device flow. |
