...
3. **Custom domain** — `auth.acme.com` en vez de `tu-slug.auth.prysmid.com`. Plan Pro+.
...
| Custom CSS | ✅ Pro+ | ✅ Pro+ | — |
| URL del login | `<slug>.auth.prysmid.com` | igual | `auth.<tu-dominio>` |
...
Por default tu instance vive en `<slug>.auth.prysmid.com`. Si querés que el login se vea bajo *tu* dominio (`auth.tuapp.com` o, en enterprise, `auth.acme.com`), seguís esto:
